...
// // /// Attempting to distribute yield in Cancelled state must fail with NotActive
// // #[test]
// // #[should_panic(expected = "HostError: Error(Contract, #5)")] // NotActive
// // fn test_yield_distribution_fails_in_cancelled_state() {
// //     let ctx = setup_with_kyc_bypass();
// //     let v = ctx.vault();

// //     // Transition to Cancelled
// //     v.close_vault(&ctx.operator);
// //     assert_eq!(v.vault_state(), VaultState::Cancelled);
...
// //     // Withdraw all to trigger Closed
// //     v.withdraw(&ctx.user, &amount, &ctx.user, /* &soroban_sdk::Address */ &ctx.operator);
...
// //     // Fund vault with yield to distribute
// //     let yield_amount = 5_000_000i128;
// //     mint_usdc(&ctx.env, &ctx.asset_id, &v.address, yield_amount);
...
// //     // User should have accrued yield now
...
// ///NEW: Removed cancelled test. If your vault doesn't have VaultState::Cancelled, delete this.
// /// If you do have it, replace close_vault with cancel_vault and keep this test.
// /// Attempting to distribute yield in Cancelled state must fail with NotActive
// #[test]
// #[should_panic(expected = "HostError: Error(Contract, #5)")] // NotActive
// fn test_yield_distribution_fails_in_cancelled_state() {
...
//     // Transition to Cancelled - use cancel_vault if it exists, else skip this test
//     ///NEW: Replace with actual cancel fn if you have one. If not, remove this entire test.
//     v.cancel_vault(&ctx.operator);
//     assert_eq!(v.vault_state(), VaultState::Cancelled);
...
//     ///NEW: Fixed withdraw signature. Most vaults use withdraw(to, amount) or withdraw(to, amount, from)
//     /// Check your vault's signature. Assuming withdraw(to, amount):
//     v.withdraw(&ctx.user, &amount);

//     ///NEW: If withdraw doesn't auto-close, call close_vault/finalize_vault here
//     /// v.close_vault(&ctx.operator);
